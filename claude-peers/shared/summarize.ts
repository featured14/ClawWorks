...
function spawnText(cmd: string, args: string[], cwd: string): Promise<{ text: string; code: number }> {
  return new Promise((resolve) => {
    const proc = spawn(cmd, args, { cwd, stdio: ["ignore", "pipe", "ignore"] });
    let text = "";
    proc.stdout.on("data", (data: Buffer) => { text += data.toString(); });
    proc.on("close", (code) => { resolve({ text, code: code ?? 1 }); });
  });
}
...
export async function getGitBranch(cwd: string): Promise<string | null> {
  try {
    const result = await spawnText("git", ["rev-parse", "--abbrev-ref", "HEAD"], cwd);
    if (result.code === 0) {
...
export async function getRecentFiles(
  ...
  try {
    ...
    const diffResult = await spawnText("git", ["diff", "--name-only", "HEAD"], cwd);

    const files = diffResult.text
      .trim()
      .split("\n")
      .filter((f) => f.length > 0);
    ...
    const logResult = await spawnText(
      "git",
      ["log", "--oneline", "--name-only", "-5", "--format="],
      cwd
    );

    const logFiles = logResult.text
      .trim()
      .split("\n")
      .filter((f) => f.length > 0);
