 ...
 *
 * Usage:
 *   tsx cli.ts status          — Show broker status and all peers
 *   tsx cli.ts peers           — List all peers
 *   tsx cli.ts send <id> <msg> — Send a message to a peer
 *   tsx cli.ts kill-broker     — Stop the broker daemon
 */
...
switch (cmd) {
  ...
  case "send": {
    ...
    if (!toId || !msg) {
      console.error("Usage: tsx cli.ts send <peer-id> <message>");
      process.exit(1);
    }
    try {
