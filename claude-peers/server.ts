 ...
 *
 * With .mcp.json:
 *   { "claude-peers": { "command": "tsx", "args": ["./server.ts"] } }
 */
...
const WORKSPACE_ID = process.env.CLAUDE_PEERS_WORKSPACE_ID ?? null;
...
async function ensureBroker(): Promise<void> {
  ...
  const tsxBin = process.env.CLAUDE_PEERS_TSX || "tsx";
  const proc = spawn(tsxBin, [BROKER_SCRIPT], {
...
function getTty(): string | null {
  try {
    ...
    if (ppid) {
      const proc = spawnSync("ps", ["-o", "tty=", "-p", String(ppid)]);
